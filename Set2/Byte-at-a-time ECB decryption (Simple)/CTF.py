...
# it proves that my own aes_ecb takes much more time than Crypto.Cipher.AES so i finally choose to use Crypto.Cipher.AES
# to decry the real world encryption
# random key generate
...
# padding
def pkcs7(string,block_size):
    tot = len(string)/block_size
    res = string[0:tot*block_size]
    r = len(string) % block_size
    if r :
        res += (string[tot*block_size:]+chr(block_size-r)*(block_size-r))
    else:
        res +=(chr(block_size)*block_size)
    return res
# api encrypt
def oracle(message):
